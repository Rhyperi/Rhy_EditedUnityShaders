...
MatcapContainer CalculateSphere(float3 inNormal, float3 inPos, sampler2D inAdd, sampler2D inMul, sampler2D inMask, float2 inUV, float inBleed, float inSign, float inAttenuation)
{
	MatcapContainer Matcap;
	float3 viewNormal = normalize(mul((float3x3)UNITY_MATRIX_V, inNormal));
    float3 viewDir = normalize(UnityWorldToViewPos(inPos));
