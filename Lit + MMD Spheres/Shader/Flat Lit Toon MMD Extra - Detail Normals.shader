	...
	SubShader
	{
		...
		Pass
		{
			...
			{			
				...
				if(light_Env != 1)
					finalColor = emissive + (_ColorIntensity * baseColor * lerp(sphereMul, maskedSphereMul, 0.5) + lerp(sphereAdd, maskedSphereAdd, 0.5)) * lerp(0, lightColor, attenuation) * lerp(toonTexColor, toonTexColorDetail, 0.5);
