	...
	SubShader
	{
		...
		Pass
		{
			...
			{			
				...
				float3 toonTexColorDetail = tex2D( _ToonTex, remappedRamp.xx);
				...
				maskedSphereAdd *= (sphereMap_var.rgb * _SphereAddIntensity) * dot(shadowTexColorDetail, grayscale_vector);
				float3 maskedSphereMul = tex2D(_SphereMulTex, maskedSphereUV);
				maskedSphereMul *= _SphereMulIntensity;
				
				float3 indirectLighting = ShadeSH9Minus + shadowTexColor + shadowTexColorDetail;
		...
		Pass
		{
			...
			float4 frag(VertexOutput i, float facing : VFACE) : COLOR
			{
				...
				float3 finalColor = (_ColorIntensity * baseColor) * (lerp(indirectLighting, directLighting, attenuation) / 2) * toonTexColor;
