	...
	SubShader
	{
		...
		Pass
		{
			...
			{			
				...
				if(light_Env != 1)
				{
					lightColor = indirectDiffuse.xyz;
					lightDirection = normalize(_DefaultLightDir);
				}
				...
				float rampValue = smoothstep(0, bw_lightDifference, bw_directLighting - bw_bottomIndirectLighting);		
				float tempValue = 0.4 * dot(normalDirection, lightDirection.xyz) + 0.5;

				float3 toonTexColor = tex2D(_ToonTex, tempValue);
				float3 shadowTexColor = tex2D(_ShadowTex, tempValue * rampValue);
				
				float3 indirectLighting = ShadeSH9Minus + shadowTexColor;
				...
				float3 finalColor = emissive + ((_ColorIntensity * baseColor * sphereMul + sphereAdd) * lightColor * toonTexColor) / 2;
				
				if(light_Env != 1)
					finalColor = emissive + (((_ColorIntensity * 2) * baseColor * sphereMul + sphereAdd) * lightColor * toonTexColor) / 2;
		...
		Pass
		{
			...
			float4 frag(VertexOutput i, float facing : VFACE) : COLOR
			{
				...
				float3 finalColor = ((_ColorIntensity * baseColor) * lightColor * toonTexColor) / 2;
				
				if(light_Env != 1)
					finalColor = (((_ColorIntensity * 2) * baseColor) * lightColor * toonTexColor) / 2;
