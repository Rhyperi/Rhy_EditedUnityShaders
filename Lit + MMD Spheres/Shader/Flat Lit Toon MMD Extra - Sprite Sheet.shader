Shader "Rhy Custom Shaders/Flat Lit Toon + MMD/Sprite Sheet"
{
	Properties
	{
		...
		_TextureSizeX("Base Texture Width", Float) = 1
		_TextureSizeY("Base Texture Height", Float) = 1
		_UVX("Sprite Width", Float) = 1
		_UVY("Sprite Height", Float) = 1
		_TravelX("UV Movement X", Range(0,1)) = 0
		_TravelY("UV Movement Y", Range(0,1)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			{			
				...
				workingUV = i.uv0;
				workingUV.x = workingUV.x / (_TextureSizeX / _UVX);
				workingUV.y = workingUV.y / (_TextureSizeY / _UVY);
				
				workingUV.x += (_TravelX);
				workingUV.y += (_TravelY);

				float4 objPos = mul(unity_ObjectToWorld, float4(0,0,0,1));
				
				i.normalDir = normalize(i.normalDir);
				float3x3 tangentTransform = float3x3(i.tangentDir, i.bitangentDir, i.normalDir);
				float3 _BumpMap_var = UnpackNormal(tex2D(_BumpMap, TRANSFORM_TEX(workingUV, _BumpMap)));
				float3 normalDirection = normalize(mul(_BumpMap_var.rgb, tangentTransform)); // Perturbed normals
				float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(workingUV, _MainTex));
				...
				float4 _EmissionMap_var = tex2D(_EmissionMap,TRANSFORM_TEX(workingUV, _EmissionMap));
				...
				float4 _ColorMask_var = tex2D(_ColorMask,TRANSFORM_TEX(workingUV, _ColorMask));
				...
				float tempValue = (0.5 * dot(normalDirection, lightDirection) + 0.5);
				...
				float3 sphereMap_var = tex2D(_SphereMap, TRANSFORM_TEX(workingUV, _SphereMap));
	...
	CustomEditor "RhyFlatLitMMDEditorSpriteSheet"
}