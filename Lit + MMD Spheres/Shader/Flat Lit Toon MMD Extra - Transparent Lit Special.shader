Shader "Rhy Custom Shaders/Flat Lit Toon + MMD/Transparent Lit Special"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag(VertexOutput i, float facing : VFACE) : COLOR
			{
				...
				float3 finalColor = (_ColorIntensity * (baseColor.rgb * toonTexColor) + (Matcap.Mul * Matcap.Add)) * (lerp(0, Lighting.directLit, attenuation));
