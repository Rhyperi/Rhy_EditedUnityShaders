	...
	SubShader
	{
		...
		Pass
		{
			...
			{			
				...
				float4 sphereSubAdd = tex2D(_SphereAddSubTex, sphereUV);
				sphereSubAdd.rgb *= ((sphereSubMap_var * (1 +_Fader)) * _SphereAddSubIntensity) * specularShadows;
				...
				float3 finalColor = emissive + (_ColorIntensity * baseColor * sphereMul + (sphereAdd + sphereSubAdd)) * (lerp(indirectLighting, directLighting, attenuation) / 2) * toonTexColor;
