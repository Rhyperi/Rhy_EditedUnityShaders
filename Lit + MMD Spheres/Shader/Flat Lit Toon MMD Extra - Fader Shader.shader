	...
	SubShader
	{
		...
		Pass
		{
			...
			{			
				...
				float3 finalColor = emissive + ((_ColorIntensity / 2) * baseColor.rgb * toonTexColor * (Matcap.Mul + (Matcap.Add + sphereSubAdd))) * (lerp(Lighting.indirectLit, Lighting.directLit, attenuation));
		...
		Pass
		{
			...
			float4 frag(VertexOutput i, float facing : VFACE) : COLOR
			{
				...
				//float3 finalColor = baseColor.rgb;
				float3 finalColor = ((_ColorIntensity) * baseColor.rgb * toonTexColor * (Matcap.Mul + (Matcap.Add + sphereSubAdd))) * (lerp(Lighting.indirectLit, Lighting.directLit, attenuation));
								
				if(light_Env != 1)
					finalColor =  ((_ColorIntensity / 2) * baseColor.rgb * toonTexColor * (Matcap.Mul + (Matcap.Add + sphereSubAdd))) * (lerp(Lighting.indirectLit, Lighting.directLit, attenuation));
