Shader "Rhy Custom Shaders/Flat Lit Toon + MMD/StealthAlt"
{
	Properties
	{
		_Stealth ("Stealth", Range(0, 1)) = 0.7378641
        _StealthScale ("StealthScale", Float ) = 2
		_StealthMask("Stealth Mask", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			uniform sampler2D _StealthMask;
			float4 _StealthMask_ST;
			...
			float4 frag(VertexOutput i, float facing : VFACE) : COLOR
			{
				...
				float4 stealthMap = tex2D(_StealthMask ,TRANSFORM_TEX(i.uv0, _StealthMask));
                ...
                float node_9302 = lerp( stealthMap.r, -stealthMap.r, _StartTopBottom) * ((_Stealth*3.0+-1.5) *_StealthScale);
                float node_3485 = (stealthMap - ((_Stealth*3.0+-1.5) *_StealthScale));
